r"""Tiny JSON-backed config for the File Toolkit GUI.

Stores just the chosen theme ("light"/"dark") and a short most-recent-first list
of files/folders the user touched.  On Windows the file lives at
``%LOCALAPPDATA%\FileToolkit\config.json``; elsewhere it falls back to
``~/.filetoolkit/config.json``.  Every function is defensive -- a corrupt or
...
def config_dir():
    ...
    ``%LOCALAPPDATA%\FileToolkit`` on Windows, ``~/.filetoolkit`` otherwise.
    """
